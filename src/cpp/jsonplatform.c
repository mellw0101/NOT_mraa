...
mraa_init_json_platform_get_index(json_object *jobj, const char *io, const char *key, int index, int *pos, int upper)
{
    ...
    if (json_object_object_get_ex(jobj, key, &jobj_temp))
    {
        ...
        *pos = (int)json_object_get_int(jobj_temp);
        if (*pos < 0 || *pos > upper)
        {
            syslog(LOG_ERR, "init_json_platform: %s %s at position: %d, gave: %d which was out of range", io, key,
                   index, *pos);
            return MRAA_ERROR_INVALID_RESOURCE;
        }
        return MRAA_SUCCESS;
...
mraa_init_json_platform_platform(json_object *jobj_platform, mraa_board_t *board, int index)
{
    ...
    ret = mraa_init_json_platform_get_pin(jobj_platform, PLATFORM_KEY, PIN_COUNT_KEY, index, &(board->phy_pin_count));
    if (ret != MRAA_SUCCESS)
    {
        return ret;
    }
    ...
    ret = mraa_init_json_platform_get_index(
        jobj_platform, PLATFORM_KEY, GPIO_COUNT_KEY, index, &(board->gpio_count), board->phy_pin_count);
    ...
    ret = mraa_init_json_platform_get_index(
        jobj_platform, PLATFORM_KEY, AIO_COUNT_KEY, index, &(board->aio_count), board->phy_pin_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ...
    ret = mraa_init_json_platform_get_index(
        jobj_platform, PLATFORM_KEY, UART_COUNT_KEY, index, &(board->uart_dev_count), MAX_UART_COUNT);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ...
    ret = mraa_init_json_platform_get_index(
        jobj_platform, PLATFORM_KEY, I2C_COUNT_KEY, index, &(board->i2c_bus_count), MAX_I2C_BUS_COUNT);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ...
    ret = mraa_init_json_platform_get_index(
        jobj_platform, PLATFORM_KEY, SPI_COUNT_KEY, index, &(board->spi_bus_count), MAX_SPI_BUS_COUNT);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ...
    ret = mraa_init_json_platform_get_pin(
        jobj_platform, PLATFORM_KEY, PWMDEFAULT_KEY, index, &(board->pwm_default_period));
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ret = mraa_init_json_platform_get_pin(jobj_platform, PLATFORM_KEY, PWMMAX_KEY, index, &(board->pwm_max_period));
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
    ret = mraa_init_json_platform_get_pin(jobj_platform, PLATFORM_KEY, PWMMIN_KEY, index, &(board->pwm_min_period));
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        return ret;
    }
...
mraa_init_json_platform_gpio(json_object *jobj_gpio, mraa_board_t *board, int index)
{
    ...
    // Get the gpio index
    ret = mraa_init_json_platform_get_index(jobj_gpio, GPIO_KEY, INDEX_KEY, index, &pos, board->phy_pin_count - 1);
    if (ret != MRAA_SUCCESS)
    {
        return ret;
    }
    // Get the gpio sysfs pin;
    ret = mraa_init_json_platform_get_pin(jobj_gpio, GPIO_KEY, RAW_PIN_KEY, index, &(board->pins[pos].gpio.pinmap));
    if (ret != MRAA_SUCCESS)
    {
        return ret;
    }
...
mraa_init_json_platform_aio(json_object *jobj_aio, mraa_board_t *board, int index)
{
    ...
    // Get the gpio sysfs pin;
    ret = mraa_init_json_platform_get_pin(jobj_aio, AIO_KEY, RAW_PIN_KEY, index, &(board->pins[pos].aio.pinmap));
    if (ret != MRAA_SUCCESS)
    {
        return ret;
    }
...
mraa_init_json_platform_i2c(json_object *jobj_i2c, mraa_board_t *board, int index)
{
    ...
    // Setup the sda pin
    ret = mraa_init_json_platform_get_index(jobj_i2c, I2C_KEY, SDAPIN_KEY, index, &pin, board->phy_pin_count - 1);
    ...
    // Setup the scl pin
    ret = mraa_init_json_platform_get_index(jobj_i2c, I2C_KEY, SCLPIN_KEY, index, &pin, board->phy_pin_count - 1);
    if (ret == MRAA_ERROR_NO_DATA_AVAILABLE)
    {
...
mraa_init_json_platform_uart(json_object *jobj_uart, mraa_board_t *board, int index)
{
    ...
    ret = mraa_init_json_platform_get_index(jobj_uart, UART_KEY, INDEX_KEY, index, &pos, board->uart_dev_count - 1);
    if (ret != MRAA_SUCCESS)
    {
        return ret;
    }
    ...
    // Setup the rx pin
    ret = mraa_init_json_platform_get_index(jobj_uart, UART_KEY, RXPIN_KEY, index, &pin, board->phy_pin_count - 1);
    if (ret == MRAA_ERROR_NO_DATA_AVAILABLE)
    {
    ...
    // Setup the TX pin
    ret = mraa_init_json_platform_get_index(jobj_uart, UART_KEY, TXPIN_KEY, index, &pin, board->phy_pin_count - 1);
    if (ret == MRAA_ERROR_NO_DATA_AVAILABLE)
    {
...
mraa_result_t
mraa_init_json_platform_loop(json_object *jobj_platform, const char *obj_key, mraa_board_t *board,
...
mraa_result_t
mraa_init_json_platform_size_check(json_object *jobj_platform, const char *obj_key, mraa_board_t *board,
                                   init_plat_func_t func, int range)
{
    json_object  *jobj_temp    = NULL;
    mraa_result_t ret          = MRAA_SUCCESS;
    int           array_length = 0;
    if (json_object_object_get_ex(jobj_platform, obj_key, &jobj_temp))
    {
        ...
        {
            syslog(LOG_ERR, "init_json_platform: The size of the %s array given was %d, max was: %d", obj_key,
                   array_length, range);
            return MRAA_ERROR_INVALID_RESOURCE;
        }
...
mraa_init_json_platform(const char *platform_json)
{
    ...
    ret = mraa_init_json_platform_size_check(
        jobj_platform, IO_KEY, board, mraa_init_json_platform_io, board->phy_pin_count);
    if (ret != MRAA_SUCCESS)
    {
        goto unsuccessful;
    }
    ...
    // Setup GPIO
    ret = mraa_init_json_platform_size_check(
        jobj_platform, GPIO_KEY, board, mraa_init_json_platform_gpio, board->phy_pin_count);
    if (ret != MRAA_SUCCESS)
    {
        goto unsuccessful;
    }
    ...
    // Setup AIO
    ret = mraa_init_json_platform_size_check(
        jobj_platform, AIO_KEY, board, mraa_init_json_platform_aio, board->phy_pin_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        goto unsuccessful;
    }
    ...
    // Setup SPI
    ret = mraa_init_json_platform_size_check(
        jobj_platform, SPI_KEY, board, mraa_init_json_platform_spi, board->spi_bus_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        goto unsuccessful;
    }
    ...
    // Setup I2C
    ret = mraa_init_json_platform_size_check(
        jobj_platform, I2C_KEY, board, mraa_init_json_platform_i2c, board->i2c_bus_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        goto unsuccessful;
    }
    ...
    // Setup UART
    ret = mraa_init_json_platform_size_check(
        jobj_platform, UART_KEY, board, mraa_init_json_platform_uart, board->uart_dev_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
    ...
    // Setup PWM
    ret = mraa_init_json_platform_size_check(
        jobj_platform, PWM_KEY, board, mraa_init_json_platform_pwm, board->phy_pin_count);
    if (ret != MRAA_SUCCESS && ret != MRAA_ERROR_NO_DATA_AVAILABLE)
    {
        goto unsuccessful;
    }
