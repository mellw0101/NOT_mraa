...
static const char paritymode_table[] = {[MRAA_UART_PARITY_NONE]  = 'N',
    ...
    if (uart_index < 0)
    {
        ...
        else
        {
            ...
            for (uart_index = 0; uart_index < number_of_uarts; uart_index++)
            {
                ...
                if (res == MRAA_SUCCESS && devpath != NULL && !strcmp(devpath, str))
                {
                    break;
                }
            }
...
{
    printf("Usage: %s { list | dev device } [ baud bps ] [ databits d ] [ parity p ] [ stopbits s ] [ ctsrts mode ] [ "
...
main(int argc, const char **argv)
{
    ...
    */
    ...
    else if (!strcmp(argv[1], "list"))
    {
        ...
        for (uart_index = 0; uart_index < number_of_uarts; uart_index++)
        {
            mraa_result_t res = mraa_uart_settings(
                uart_index, &name, &dev, &baudrate, &databits, &stopbits, &parity, &ctsrts, &xonxoff);
            if (res == MRAA_SUCCESS)
            {
                if (name == NULL)
                {
                ...
                printf("%i. %-12s %-16s %7i %i%c%i %s %s\n", uart_index, name, dev, baudrate, databits,
                       paritymode_table[parity], stopbits, ctsrts ? "CTSRTS" : "(none)",
                       xonxoff ? "XONXOFF" : "(none) ");
            }
            else
            {
    ...
    else
    {
                        ...
                        if (!strcmp(argv[i], "stopbits"))
                        {
                            ...
                            if (!strcmp(argv[i], "databits") || !strcmp(argv[i], "bytesize"))
                            {
                                ...
                                if (!strcmp(argv[i], "paritybits") || !strcmp(argv[i], "parity"))
                                {
                                    ...
                                    /* Even if there were gaps between declared array elements, they would be
                                     * initialized to 0 compile time */
                                    ...
                                    {
                                        fprintf(
                                            stderr, "warning: unrecognized parity mode %s (ignoring)\n", argv[i + 1]);
                                    }
                                    if (uart != NULL)
                                    {
                                        mraa_uart_set_mode(uart, databits, parity, stopbits);
                                    ...
                                    if (!strcmp(argv[i], "show"))
                                    {
                                        ...
                                        if (!strcmp(argv[i], "send"))
                                        {
                                            ...
                                            if (!strcmp(argv[i], "receive") || !strcmp(argv[i], "recieve") ||
                                                !strcmp(argv[i], "recv"))
                                        {
                                            if (i + 1 >= argc)
                                            {
                                                fprintf(
                                                    stderr,
                                                    "%s : %s needs a timeout (in seconds, fractional ok) as argument\n",
                                                    argv[0], argv[i]);
                                                break;
                                            }
                                            recieve         = TRUE;
        ...
        if (i == argc && uart != NULL)
        {
            ...
            if (show)
            {
                ...
                dev = mraa_uart_get_dev_path(uart);
                mraa_uart_settings(-1, &dev, &name, &baudrate, &databits, &stopbits, &parity, &ctsrts, &xonxoff);
                printf("%-12s %-16s %7i %i%c%i %s %s\n", name != NULL ? name : undefined, dev, baudrate, databits,
                       paritymode_table[parity], stopbits, ctsrts ? "CTS/RTS" : "(no hw)",
                       xonxoff ? "XONXOFF" : "(no sw)");
            }
        }
        else
        {
