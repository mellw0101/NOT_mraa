...
#include "../include/api/i2c.h"
#include "../include/api/pwm.h"
#include "../include/api/spi.h"
#include "../include/api/uart.h"
// #include "../include/firmata_mraa.h"
// #include "../include/gpio.h"
#include "../include/gpio_chardev.h"
#include "../include/grovepi.h"
...
imraa_init()
{
    ...
    syslog(LOG_NOTICE, "libmraa version %s initialised by user '%s' with EUID %d", mraa_get_version(),
           (proc_user != NULL) ? proc_user->pw_name : "<unknown>", proc_euid);
...
mraa_gpio_lookup(const char *pin_name)
{
    ...
    for (i = 0; i < plat->phy_pin_count; i++)
    {
        ...
        if (plat->pins[i].name != NULL && strncmp(pin_name, plat->pins[i].name, strlen(plat->pins[i].name) + 1) == 0)
        {
            return i;
        }
    }
