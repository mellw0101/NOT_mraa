...
#include "../include/intel_edison_fab_c.h"
...
static unsigned int                 outputen[] = {
    248, 249, 250, 251, 252, 253, 254, 255, 256, 257, 258, 259, 260, 261, 232, 233, 234, 235, 236, 237};
static mraa_gpio_context agpioOutputen[sizeof(outputen) / sizeof(outputen[0])];

static unsigned int pullup_map[] = {
    216, 217, 218, 219, 220, 221, 222, 223, 224, 225, 226, 227, 228, 229, 208, 209, 210, 211, 212, 213};
...
static mraa_edison_pwm_wa_pinstate_t pwm_wa_state[4] = {
    {.duty_cycle = 0, .pwm_disabled = 0}
};
...
mraa_intel_edison_misc_spi()
{
    ...
    static const int gpio_dir_list[] = {
        MRAA_GPIO_OUT, MRAA_GPIO_OUT, MRAA_GPIO_OUT, MRAA_GPIO_OUT, MRAA_GPIO_OUT, MRAA_GPIO_OUT};
    int           i;
    mraa_result_t ret;
...
mraa_intel_edison_aio_get_fp(mraa_aio_context dev)
{
    ...
    {
        syslog(LOG_ERR,
               "edison: Failed to open Analog input raw file %s for "
               "reading!",
               file_path);
        return MRAA_ERROR_INVALID_RESOURCE;
    }
...
is_arduino_board()
{
    ...
    for (i = 0; i < (sizeof(gpiochip_idx) / sizeof(gpiochip_idx[0])); i++)
    {
        ...
        snprintf(gpiochip_path, MAX_SIZE, SYSFS_CLASS_GPIO "/gpiochip%d/label", gpiochip_idx[i]);
        FILE *fp;
        fp = fopen(gpiochip_path, "r");
        if (fp == NULL)
        {
            syslog(LOG_INFO, "edison: could not open '%s', errno %d", gpiochip_path, errno);
            return 0;
        }
        ...
        if (ret != 1)
        {
            syslog(LOG_INFO, "edison: could not read from '%s', errno %d", gpiochip_path, errno_saved);
            return 0;
        }
        ...
        {
            syslog(LOG_INFO, "edison: gpiochip label (%s) is not what we expect (%s)\n", gpiochip_label,
...
mraa_intel_edison_fab_c()
{
    ...
    {
        vanilla_kernel = 1;
        syslog(LOG_NOTICE, "edison: Linux version 4 or higher detected, assuming Vanilla kernel");
    };

    if (is_arduino_board() == 0)
    {
        syslog(LOG_NOTICE, "edison: Arduino board not detected, assuming Intel Edison Miniboard");
