...
#define for_each_gpio_group(group, dev)                                       \
    for (int k = 0; k < dev->num_chips && (group = &dev->gpio_group[k]); ++k) \
        if (dev->gpio_group[k].is_required)

#define for_each_gpio_chip(cinfo, cinfos, num_chips) \
    for (int idx = 0; idx < num_chips && (cinfo = cinfos[idx]); (idx++))
...
struct _i2c
{
    ...
    unsigned long
        funcs;  /**< /dev/i2c-* device capabilities as per https://www.kernel.org/doc/Documentation/i2c/functionality */
...
typedef enum
{
    PINCMD_UNDEFINED = 0, // do not modify, default command for zero value, used for backward compatibility with boards
                          // where pincmd is not defined (it will be deleted later)
