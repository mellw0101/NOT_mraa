...
static const char *spilink[] = {"/sys/class/spidev/spidev0.0", "/sys/class/spidev/spidev1.0"};

static const char *i2clink[] = {"/sys/class/i2c-dev/i2c-0", "/sys/class/i2c-dev/i2c-1"};

static unsigned char regIon[16] = {
    0x2A, 0x2D, 0x30, 0x33, 0x36, 0x3B, 0x40, 0x45, 0x4A, 0x4D, 0x50, 0x53, 0x56, 0x5B, 0x60, 0x65};

static unsigned int IonValue[16] = {
...
pwm_period_replace(mraa_pwm_context dev, int period)
{
    _tperiod = period;
    return MRAA_SUCCESS;
}
...
static float
pwm_read_replace(mraa_pwm_context dev)
{
...
static mraa_result_t
pwm_write_replace(mraa_pwm_context dev, float duty)
{
    unsigned char rx_tx_buf[3] = {0};
...
static mraa_result_t
pwm_enable_replace(mraa_pwm_context dev, int enable)
{
    int pin = dev->pin;
...
static mraa_result_t
pwm_init_raw_replace(mraa_pwm_context dev, int pin)
{
    char buffer[100] = {0};
    int  i, fd;
