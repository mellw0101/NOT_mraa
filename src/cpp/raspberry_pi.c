...
mmap_reg_addr(unsigned long base_addr)
{
    ...
    /* mmap IO */
    reg_addr_map = mmap(NULL,                               // Any adddress in our space will do
...
mraa_raspberry_pi_mmap_write(mraa_gpio_context dev, int value)
{
    if (value)
    {
        *(volatile uint32_t *)(mmap_reg + BCM283X_GPSET0 + (dev->pin / 32) * 4) = (uint32_t)(1 << (dev->pin % 32));
    }
    else
    {
...
mraa_raspberry_pi_mmap_setup(mraa_gpio_context dev, mraa_boolean_t en)
{
    ...
    if (mmap_reg == NULL)
    {
        ...
        mmap_reg = (uint8_t *)mmap(
            NULL, block_size, PROT_READ | PROT_WRITE, MAP_FILE | MAP_SHARED, mmap_fd, peripheral_base + GPIO_OFFSET);
...
mraa_raspberry_pi()
{
    ...
    if (fh != NULL)
    {
        while (getline(&line, &len, fh) != -1)
        {
            if (strncmp(line, "Revision", 8) == 0)
            {
                ...
                else if (strstr(line, "0004") || strstr(line, "0005") || strstr(line, "0006") || strstr(line, "000d") ||
                         strstr(line, "000e") || strstr(line, "000f"))
                {
                ...
                else if (strstr(line, "a02082") || strstr(line, "a020a0") || strstr(line, "a22082") ||
                         strstr(line, "a32082"))
                {
                ...
                else if (strstr(line, "a03111") || strstr(line, "a03112") || strstr(line, "a03115") ||
                         strstr(line, "b03111") || strstr(line, "b03112") || strstr(line, "b03114") ||
                         strstr(line, "b03115") || strstr(line, "c03111") || strstr(line, "c03112") ||
                         strstr(line, "c03114") || strstr(line, "c03115") || strstr(line, "d03114") ||
                         strstr(line, "d03115"))
                {
    ...
    if ((platform_detected == PLATFORM_RASPBERRY_PI_A_REV_2) || (platform_detected == PLATFORM_RASPBERRY_PI_B_REV_2))
    {
    ...
    if ((platform_detected == PLATFORM_RASPBERRY_PI_A_PLUS_REV_1) ||
        ...
        (platform_detected == PLATFORM_RASPBERRY_PI_ZERO) || (platform_detected == PLATFORM_RASPBERRY_PI_ZERO_W) ||
        (platform_detected == PLATFORM_RASPBERRY_PI3_B_PLUS) || (platform_detected == PLATFORM_RASPBERRY_PI3_A_PLUS) ||
        (platform_detected == PLATFORM_RASPBERRY_PI4_B) || (platform_detected == PLATFORM_RASPBERRY_PI_400) ||
        (platform_detected == PLATFORM_RASPBERRY_PI5_B))
    {
