...
#include "../include/api/initio.h"
#include "../include/initio_keys.h"
...
parse_gpio(char **proto, size_t n, const char *proto_full)
{
    ...
    if (mraa_atoi_x(proto[idx], NULL, &value, 0) == MRAA_SUCCESS)
    {
        ...
        {
            syslog(LOG_ERR, "parse_gpio: could not init gpio number '%d' with value '%d' from '%s'", gpio_num, value,
                   proto_full);
            mraa_gpio_close(dev);
            return NULL;
        }
...
mraa_io_init(const char *strdesc, mraa_io_descriptor **desc)
{
    ...
    for (int i = 0; i < num_descs; ++i)
    {
        ...
        else if (strncmp(str_tokens[0], GPIO_KEY, strlen(GPIO_KEY)) == 0 && strlen(str_tokens[0]) == strlen(GPIO_KEY))
        {
...
#if !defined(PERIPHERALMAN)
        else if (strncmp(str_tokens[0], IIO_KEY, strlen(IIO_KEY)) == 0 && strlen(str_tokens[0]) == strlen(IIO_KEY))
        {
...
#endif
        else if (strncmp(str_tokens[0], I2C_KEY, strlen(I2C_KEY)) == 0 && strlen(str_tokens[0]) == strlen(I2C_KEY))
        {
        ...
        else if (strncmp(str_tokens[0], PWM_KEY, strlen(PWM_KEY)) == 0 && strlen(str_tokens[0]) == strlen(PWM_KEY))
        {
        ...
        else if (strncmp(str_tokens[0], SPI_KEY, strlen(SPI_KEY)) == 0 && strlen(str_tokens[0]) == strlen(SPI_KEY))
        {
        ...
        else if (strncmp(str_tokens[0], UART_KEY, strlen(UART_KEY)) == 0 && strlen(str_tokens[0]) == strlen(UART_KEY))
        {
