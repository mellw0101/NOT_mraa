        ...
        if (strcmp(ent->d_name + strlen(ent->d_name) - strlen("_index"), "_index") == 0)
        {
            ...
            if (fd != -1)
            {
                ...
                if (fd != -1)
                {
                    ...
                    ret = sscanf(
                        readbuf, "%ce:%c%u/%u>>%u", &shortbuf, &signchar, &chan->bits_used, &padint, &chan->shift);
...
mraa_result_t
mraa_iio_event_extract_event(struct iio_event_data *event, int *chan_type, int *modifier, int *type, int *direction,
                             int *channel, int *channel2, int *different)
{
