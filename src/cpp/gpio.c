...
mraa_gpio_chardev_init(int pins[], int num_pins)
{
    ...
    for (int i = 0; i < num_pins; ++i)
    {
        ...
        {
            syslog(LOG_ERR, "[GPIOD_INTERFACE]: init: pin %d beyond platform pin count (%d)", pins[i],
                   board->phy_pin_count);
            mraa_gpio_close(dev);
            return NULL;
        }
...
static mraa_result_t
mraa_gpio_wait_interrupt(int fds[], int num_fds
#ifndef HAVE_PTHREAD_CANCEL
                         ,
                         int control_fd
#endif
...
mraa_gpio_interrupt_handler(void *arg)
{
    ...
    else
    {
        ...
        while (it)
        {
            ...
            if (fps[idx] < 0)
            {
                syslog(LOG_ERR, "gpio%i: interrupt_handler: failed to open 'value' : %s", it->pin, strerror(errno));
...
mraa_gpio_edge_mode(mraa_gpio_context dev, mraa_gpio_edge_t mode)
{
    ...
    while (it)
    {
        ...
        if (edge == -1)
        {
            syslog(LOG_ERR, "gpio%i: edge_mode: Failed to open 'edge' for writing: %s", it->pin, strerror(errno));
            return MRAA_ERROR_INVALID_RESOURCE;
        }
...
mraa_gpio_mode(mraa_gpio_context dev, mraa_gpio_mode_t mode)
{
    ...
    if (plat->chardev_capable)
    {
        ...
        {
            line_handle =
                mraa_get_lines_handle(gpio_iter->dev_fd, gpio_iter->gpio_lines, gpio_iter->num_gpio_lines, flags, 0);
            if (line_handle <= 0)
            {
    ...
    while (it)
    {
        ...
        if (direction == -1)
        {
            ...
            switch (dir)
            {
                ...
                default :
                    syslog(
                        LOG_ERR, "gpio%i: dir: Failed to open 'direction' for writing: %s", it->pin, strerror(errno));
                    return MRAA_ERROR_INVALID_RESOURCE;
            }
        }
...
mraa_gpio_read_multi(mraa_gpio_context dev, int output_values[])
{
    ...
    if (plat->chardev_capable)
    {
        ...
        for_each_gpio_group(gpio_iter, dev)
        {
            ...
            {
                gpio_iter->gpiod_handle = mraa_get_lines_handle(
                    gpio_iter->dev_fd, gpio_iter->gpio_lines, gpio_iter->num_gpio_lines, flags, 0);
                if (gpio_iter->gpiod_handle <= 0)
                {
            ...
            status = mraa_get_line_values(gpio_iter->gpiod_handle, gpio_iter->num_gpio_lines, gpio_iter->rw_values);
            if (status < 0)
            {
...
mraa_gpio_write_multi(mraa_gpio_context dev, int input_values[])
{
    ...
    if (plat->chardev_capable)
    {
        ...
        for_each_gpio_group(gpio_iter, dev)
        {
            ...
            status = mraa_set_line_values(gpio_iter->gpiod_handle, gpio_iter->num_gpio_lines, gpio_iter->rw_values);
            if (status < 0)
            {
                syslog(LOG_ERR, "[GPIOD_INTERFACE]: error writing gpio");
                return MRAA_ERROR_INVALID_RESOURCE;
